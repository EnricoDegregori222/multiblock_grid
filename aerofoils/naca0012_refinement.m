...
xnaca(kkk+2:end-nllte,:) = xxnaca(end:-1:1,:);
ynaca(kkk+2:end-nllte,:) = yynaca(end:-1:1,:);

% figure(2)
% plot(xnaca(:,1),ynaca(:,1), '-or'); hold on; grid on;
% plot(xnaca(:,2),ynaca(:,2), '-ob');

xend(:,1) = [xnaca(1:kkk+1,1); xnaca(kkk+3:end-nllte,1); xnaca(end-nllte+1:end,1)];
xend(:,2) = [xnaca(1:kkk+1,2); xnaca(kkk+3:end-nllte,2); xnaca(end-nllte+1:end,2)];
yend(:,1) = [ynaca(1:kkk+1,1); ynaca(kkk+3:end-nllte,1); ynaca(end-nllte+1:end,1)];
yend(:,2) = [ynaca(1:kkk+1,2); ynaca(kkk+3:end-nllte,2); ynaca(end-nllte+1:end,2)];
...
% figure(3)
% plot(xnaca(1:kkk+1,2), ynaca(1:kkk+1,2), '-or'); hold on; grid on;
% plot(xnaca(kkk+3:end-nllte,2),ynaca(kkk+3:end-nllte,2), '-ob')
% plot(xnaca(kkk+2,2),ynaca(kkk+2,2), '-ok')


fileID = fopen('naca0012_ref.dat','w');
